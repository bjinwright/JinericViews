...
def save_instance(form, model, instance, fields=None, fail_message='saved',
    ...
    opts = model._properties.iteritems()
    if form.errors:
    ...
    local_field_names = [k for k,f in opts]
...
def model_to_dict(instance, fields=None, exclude=None):
    ...
    from google.appengine.ext import db
    opts = instance._properties.iteritems()
#    for k,f in opts:
#        print k,f
    print dir(instance)
    data = {'__key__': instance.key()}
    for k,f in opts:
        ...
        if exclude and k in exclude:
            continue
        if k == 'parent':
            data[k] = instance.parent_key()
        else:
            data[k] = f.get_value_for_form(instance)
    return data

def fields_for_model(model, fields=None, exclude=None, formfield_callback=lambda f: f.get_form_field()):
    """
    ...
    for f,k in opts :
        ...
        if exclude and f in exclude:
            continue
        formfield = formfield_callback(k)
        if formfield:
...
class ModelMultipleChoiceField(ModelChoiceField):
    ...
    def clean(self, value):
        ...
        final_values = self.queryset._model_class.get(value)
